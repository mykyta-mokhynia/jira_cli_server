...
interface AuditResult {
    ...
    groups: {
        missing: string[];
        existing: string[];
        incorrect: string[]; // Logic for incorrect naming might be complex, skipping for MVP or just listing unknown groups
    };
    permissionScheme: {
        current: string;
        target: string;
...
export const auditProject = async (projectKey: string, projectType: 'team' | 'non-team'): Promise<AuditResult> => {
    ...
    const projectGroupsSearch = await jira.groups.findGroups({ query: projectKey });
    const projectGroupsList = (projectGroupsSearch as any).groups || projectGroupsSearch || [];
    const projectGroupNames = projectGroupsList.map((g: any) => g.name || '');
    ...
    for (const tg of templateGroups) {
        if (projectGroupNames.includes(tg)) {
            ...
            const exactSearch = await jira.groups.findGroups({ query: tg });
            const exactList = (exactSearch as any).groups || exactSearch || [];
            if (exactList.some((g: any) => g.name === tg)) {
                existingGroupNames.push(tg);
            }
        }
    }
    ...
    console.log(`[Audit] Project Style:`, (project as any).style);
...
export const applyProjectTemplate = async (projectKey: string, projectType: string) => {
    // 1. Create Groups
    const templateGroups = [
        `${projectKey}group`,
        `${projectKey}group_admin`,
    ...
    for (const groupName of templateGroups) {
        ...
        try {
            ...
            const exists = groupsList.some((g: any) => g.name === groupName);
    ...
    let targetPermSchemeId: number | undefined;

    // Check if target exists
    const permSchemes = await jira.permissionSchemes.getAllPermissionSchemes();
    const existingPermScheme = permSchemes.permissionSchemes?.find((s: any) => s.name === targetPermSchemeName);
    ...
    const secSchemes = await jira.issueSecuritySchemes.getIssueSecuritySchemes();
    const targetSecScheme = secSchemes.issueSecuritySchemes?.find((s: any) => s.name === targetSecSchemeName);
    ...
    await migrateUsersAndCleanup(jira, projectKey, roleMapping, templateGroups);

    return { success: true, projectKey, projectType };
};

const migrateUsersAndCleanup = async (jira: Version3Client, projectKey: string, roleMapping: { [group: string]: string }, templateGroups: string[]) => {
    console.log(`[Migration] Starting User Migration & Cleanup for ${projectKey}...`);

    const usersToMigrate = new Map<string, { accountId: string, highestRole: number }>();
    ...
    for (const roleName of Object.values(roleMapping)) {
        ...
        try {
            ...
            for (const actor of currentActors) {
                ...
                const actorUser = (actor as any).actorUser;
                ...
                if (actorType === 'atlassian-group-role-actor') {
                    ...
                    if (!templateGroups.includes(name)) {
                        if (name === 'org-admins' || name === 'audit') continue;
                        ...
                        try {
                            ...
                            for (const user of members) {
                                if (!user.accountId || user.accountType === 'app') continue;
    ...
    try {
        ...
        ((oaMembers as any).values || []).forEach((u: any) => {
            if (u.accountId) orgAdmins.add(u.accountId);
        });
    } catch (e) {
...
function updateHighestRole(map: Map<string, { accountId: string, highestRole: number }>, accountId: string, priority: number) {

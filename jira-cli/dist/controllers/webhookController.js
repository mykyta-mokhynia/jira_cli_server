...
exports.handleIssueWebhook = void 0;
const WatcherService = __importStar(require("../services/watcherService"));
const WATCHERS_FIELD_ID = "customfield_10170"; // Change if needed based on user config
const handleIssueWebhook = async (req, res) => {
    try {
        const data = req.body;
        ...
        const currentWatcherAccountIds = new Set();
        if (Array.isArray(fieldVal)) {
            fieldVal.forEach((user) => {
                if (user.accountId)
                    currentWatcherAccountIds.add(user.accountId);
            });
        }
...
exports.handleIssueWebhook = handleIssueWebhook;
